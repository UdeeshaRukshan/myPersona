const projects = [
    {
      id: 1,
      title: "HealthMate",
      description: "NFC-Based Patient Tracking system for hospitals to manage patient data securely.",
      image: "/Healthmate.png",  // Store images in public/projects/
      github: "https://github.com/yourusername/waste-management",
      demo: "https://waste-demo.com",
    },
    {
      id: 2,
      title: "Education management system",
      description: "A web-based platform to manage educational institutions' data and operations. by using microservice architecture. ",
      image: "/Skillhub.png",
      github: "https://github.com/UdeeshaRukshan/DS_Assignment_",
      demo: "https://nfc-demo.com",
    },
    {
      id: 3,
      title: "Nasa Space Discover",
      description: "a hosted webapp that used nasa api to show the latest news and images from space.",
      image: "/apod.jpg",
      github: "https://github.com/yourusername/ai-learning",
      demo: "https://af-frontend-beryl.vercel.app/profile",
    },
    {
      id: 4,
      title: "Ai Avatar Generator",
      description: "A hosted ai avartar generator that takes user input and generates a unique avartar using huggingface model.",
      image: "/image-7.png",
      github: "https://github.com/UdeeshaRukshan/AiAvatarDeploy",
      demo: "https://ai-avatar-deploy-ochre.vercel.app",
    },
    {
      id: 5,
      title: "Code complexity analyzer",
      description: "A tool to analyze the complexity of java code and suggest improvements.",
      image: "/Healthmate.png",
      github: "https://github.com/UdeeshaRukshan/Code_Complexity",
      demo: "https://ai-learning-demo.com",
    },
    {
      id: 6,
      title: "AuctionPal",
      description: "A webapp to manage auctions and bidding for products.",
      image: "/Healthmate.png",
      github: "https://github.com/UdeeshaRukshan/ITP-PROJECT-U-2023",
      demo: "https://ai-learning-demo.com",
    },
    
   
  ];
  export default projects;
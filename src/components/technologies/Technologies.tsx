...
const techIcons = [
	...
	},
	{
		...
		darkInvert: true,
	}, // Assuming nextjs logo might need invert for dark
	{
		name: "Node.js",
...
const TechIcon: React.FC<{ icon: typeof techIcons[0]; index: number }> = ({
	icon,
	index,
}) => {
	return (
		<motion.div
			className={`flex-shrink-0 w-20 h-20 md:w-24 md:h-24 p-3 bg-white dark:bg-gray-700 rounded-lg shadow-md flex items-center justify-center transition-all duration-300 transform hover:scale-110 hover:shadow-xl ${
			...
			transition={{
				duration: 0.3,
				delay: (index % techIcons.length) * 0.05,
				ease: "easeOut" as const,
			}} // Stagger based on original index
